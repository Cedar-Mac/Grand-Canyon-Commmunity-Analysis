...
3 files seem to get duplicated with the duplicates being all zero's. The three files are BA_2_S61_PE.1, CR_226_1_S47_PE.1, and DIA_2_S88_PE.1. Issue resolved.
```{r}
...
## NMS of the number of ESV's per OTU in method space space. 
```{r}
...
entropy_score <- taxreturn::codon_entropy(haplo_seq_StringSet, genetic_code = "SGC4", codon_filter = TRUE, method = "ML")
#method "ML" is the maximum likelyhood. Calculates Shannon Entropy (H) using frequencies of bases from the reference CO1 gene?
#genetic code "SGC4" is the invertebrate mitochondrial reference genome
```

Some sequences are being removed because they contain stop codons. These are clearly not true haplotypes.
